...
def reflect_episode(conn: sqlite3.Connection, episode_id: int, max_patches: int = 3) -> list[str]:
        ...
        evidence.update(
            {
                ...
                "summary": diff.content or row["summary"],
                "content": diff.content or episode_text(messages),
                "artifact_paths": artifact_paths,
            }
        )
        proposal = propose_patch(load_state(), evidence, source=f"reflection:episode:{episode_id}")
    ...
    conn.execute(
        ...
        VALUES ('episode_reflected', ?, ?, 'soul reflect episode')
        """,
        (
            dumps_json(
                {
            ...
            f"Reflected episode {episode_id}: {', '.join(diff.diff_type for diff in diffs) if diffs else 'NO_CHANGE'}",
        ),
    )
    conn.commit()
    return patch_ids
